    ...
    const SubmitHandler = async (e) => {
        ...
        try {
            const { data } = await axios.post('http://localhost:3005/register', {
                Name, Password, Email, Club, Designation, Admin
            }, {
                headers: {
                    'Content-Type': 'application/json'
                }
                    ...
                    <div id={"login-body"}>
                        <form id={"login-form"} onSubmit={SubmitHandler}>
                            <div className="aa">
                                <div className="aa1">
                                    <label htmlFor="panel">
                                        <input type="radio" name="checkbox" value="Admin" checked={Admin === 'Admin'}
          onChange={handleOptionChange}/> Admin
                                    </label>
                                </div>
                                <div className='aa1'>
                                    <label htmlFor="panel">
                                        <input type="radio" name="checkbox" value="Non-Admin" checked={Admin === 'Non-Admin'}
          onChange={handleOptionChange}/> Non-Admin
                                    </label>
                                </div>
                            </div>
                        <label htmlFor="email" id="email">Name </label>
                            <input id="email" type={"text"} placeholder={"Name"} required value={Name} onChange={(e) => { setName(e.target.value) }} />
                        <label htmlFor="email" id="email">Email </label>
                            <input id="email" type={"text"} placeholder={"Email"} required value={Email} onChange={(e) => { setEmail(e.target.value) }} />
                        <label htmlFor="password" id="email">Password </label>
                            <input id="password" type={"password"} placeholder={"Password"} required value={Password} onChange={(e) => { setPassword(e.target.value) }} />
                            <label htmlFor="club" id="email">Club </label>
                            <select name="club" id="Email" value={Club} onChange={(e) => { setClub(e.target.value) }}>
                            ...
                            <label htmlFor="Designation" id="email">Designation</label>
                            <input id="email" type={"text"} placeholder={"Designation"} required value={Designation} onChange={(e) => { setDesignation(e.target.value) }} />
                            <div id={"login-button"}>
